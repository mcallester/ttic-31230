...
\bigskip
\bigskip
Here we will show that for the continuous case with $p_\Phi(z|y)$ and $\hat{p}_\Phi(z)$ both Gaussian, we can assume without loss
of generality that
\bigskip
...
\slide{Summary: Gaussian RDAs}


\vfill
Gaussian RDA: {\color{red} $z_\Phi(y,\epsilon) = \mu_\Phi(y) + \sigma_\Phi(y) \odot \epsilon$,\hspace{2em} $\epsilon \sim {\cal N}(0,I)$}

{\color{red}
\begin{eqnarray*}
\Phi^* & = & \argmin_\Phi E_{y \sim \pop}\left(\begin{array}{l}\;\;\; \;KL(p_\Phi(z|y),{\cal N}(0,I)) \\
\\
+ \;\;\;\lambda E_\epsilon\; \mathrm{Dist}(y,y_\Phi(z_\Phi(y,\epsilon))) \end{array}\right)
\end{eqnarray*}
}
