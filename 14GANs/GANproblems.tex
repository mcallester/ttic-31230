...
\solution{
  ...
  For $N \geq 2$ let $\tilde{P}_\Phi^{(N)}$ be the distribution on tuples $\tuple{i,y_1,\ldots,y_N,x}$ defined by drawing a pair $\tuple{x,y}$ from $\pop$,
  where $y$ is a positive sample of $\pop(y|x)$,
  drawing IID negatives from $P_\Phi(y|x)$; then
  inserting the positive at a random position among the negatives; and
  returning $(i,y_1,\ldots,y_N,x)$ where $i$ is the index of the positive.
