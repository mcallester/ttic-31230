...
Consider a population probability distribution $\mathrm{Pop}$ on a discrete variable $y$.
...
\slide{Cross Entropy Coding Theorem}

Consider \
$$H_2(P,Q) = E_y \;-\log_2\;Q(y) = H(P,Q)/(\ln 2)$$

\vfill
This can be interpreted as the number of bits used to code draws from $P$ when using the optimal for distribution $Q$.
