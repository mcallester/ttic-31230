...
\slide{Train Data, Validation Data and Test Data}
...
\slideplain{Loss Vs. Error Rate (or BLEU Score)}
...
\slide{$L_2$ Regularization (Weight Decay or Shrinkage)}
...
\slide{Conjugacy Considerations (TZ)}
...
\slide{Batch Scaling (TZ)}

{\color{red} $$\Phi \;\minuseq\; \eta\left(\hat{g} +\frac{\lambda}{N}\Phi\right)$$}

\vfill
Setting {\color{red} $\eta = B\eta'$} will now handle batch size scaling using the batch size independent learning rate {\color{red} $\eta'$}.

\slide{Efficiency and Numerical Stability}
...
\slide{A Generalization Guarantee Motivating Shrinkage}
