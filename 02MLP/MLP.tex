...
\slide{Vector Transpose in Linear Algebra} 
...
\slide{Inner and Outer Products in Linear Algebra}
...
\mathrm{Einstein} & & {\color{red} \tilde{h}[j]} & = & {\color{red} \sum_i\;W[j,i] \;x[i]} \\
\\
& &  {\color{red} h[j]} & = & {\color{red} \sigma\left(\tilde{h}[j] - B[j]\right)} \\
  \\
\mathrm{Loop:} & \mathrm{for}\;j,i & {\color{red} \tilde{h}[j]} & \pluseq & {\color{red} W[j,i]x[i]} \\
\\
