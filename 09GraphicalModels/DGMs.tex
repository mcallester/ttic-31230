...
\vfill
It seems that constrastive divergence is just an improperly weighted version pseudolikelihood.
...
\vfill
{\color{red} $${\cal L} = s(y[i=c]) - s(y)$$}

Since the loss is directly computed, we can back-propagate on the loss.

\slide{Contrastive Divergence Theorem}

If $P_s(\hat{y}) = \mathrm{Pop}$ then
{\color{red} $$E_{y \sim \pop} \;s(y) = E_{y \sim \pop}\; s(y[i=c])$$}
and hence the expected update is zero.

\vfill
If $P_s = \pop$ then the CD updates has no net gradient.
}
