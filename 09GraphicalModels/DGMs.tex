...
\\
& = & P_s(\hat{y}) + \sum_{\hat{y}' \in N(\hat{y})}\;P_s(\hat{y}')\frac{P_{\mathrm{Trans}}(\hat{y}\;|\;\hat{y})}{N_\mathrm{nodes}}
- P_s(\hat{y})\frac{P_{\mathrm{Trans}}(\hat{y}'\;|\;\hat{y})}{N_{\mathrm{nodes}}}
\end{eqnarray*}
